...
export class Handshake {
    ...
    public static async initiate(myIdentity: Identity): Promise<{ ephemeralKeyPair: any, packet: Buffer }> {
        await _sodium.ready;
    ...
    public static async respond(packet: Buffer, myIdentity: Identity): Promise<{ session: ISession, responsePacket: Buffer, peerId: string }> {
        await _sodium.ready;
        const sodium = _sodium;

        if (packet.length !== 128) throw new Error("Invalid Handshake packet length");
    ...
    public static async finish(packet: Buffer, myIdentity: Identity, myEphemeralKeyPair: any): Promise<{ session: ISession, peerId: string }> {
        await _sodium.ready;
        const sodium = _sodium;
